...
После запуска (github Pages) в "Загрузить пример RDF данных" выбрать например, Turtle в кнопка "Визуализировать". Будет построен граф. Управлять видимостью эдементов графа можно через фильтры ("Фильтры типов:"), в данном варианте только через типы предикатов, но можно дополнить и фильтрацию через типы \ классы объектов-сущностей (субъектов).  
